...
def RNN(x, weights, biases):
    ...
    outputs, states = rnn.rnn(lstm_cell, x, dtype=tf.float32, clock = [1, 2, 4, 8, 16, 32])		# outputs : 40 * batch_size * 98	# state  : batch_size * 196
...
with tf.Session() as sess:
    ...
    while step_all * batch_size < training_iters:
        ...
        '''
        print(batch_x.shape)
        a = tf.transpose(batch_x, [1, 0, 2])
        print(a)
        a = tf.reshape(a, [-1, n_input])
        print(a)    
        a = tf.split(0, n_steps, a)   
        print(a) 
        exit()
        '''
