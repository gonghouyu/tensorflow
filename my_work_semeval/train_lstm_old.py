...
def RNN(x, weights, biases):
    ...
    outputs, states = rnn.rnn(lstm_cell, x, dtype=tf.float32)		# outputs : 40 * batch_size * 98	# state  : batch_size * 196
...
with tf.Session() as sess:
    ...
    writer = tf.train.SummaryWriter('tensorboard/lstm_old/exp_1', sess.graph)
    sess.run(init)
    step = 0
    step_all = 0
    # train
