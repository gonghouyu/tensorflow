...
def RNN(x, weights, biases):
    ...
    # dropout
    outputs_all_drop = outputs_all
    outputs_all_drop = tf.nn.dropout(outputs_all_drop, keep_prob=0.5)

    M = tanh(tf.matmul(outputs_all_drop,W_h))    # (N*batch) * 2*hidden
    ...
    for i in range(batch_s):
        ...
        att = tf.reshape(a_2,[1, n_steps])            # 1*N
        out = tf.reshape(o_2,[n_steps,2*n_hidden])    # N*2*n_hidden
    ...
    #---------------------------------------------------------
    #_h = W_p*r + W_x*outputs[-1]

    # dropout
    r = tf.nn.dropout(r, keep_prob=0.25)

    predict = tf.matmul(r, weights['out']) + biases['out']
